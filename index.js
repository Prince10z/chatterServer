...
io.on("connection", (socket) => {
    ...
    socket.on('join-room', async ({ roomId, username }) => {
        ...
        console.log(roomId);
        if (!roomId || !username) {
            return socket.emit(`error-${socket.id}`, 'Please provide a room ID and username');
        }
        try {
            ...
            if (test.status == "success") {
                ...
                socket.broadcast.emit(`${roomId}-join`, `New user is connected: ${username}`);
            } else {
            ...
            socket.emit(`error-${socket.id}`, 'An error occurred while joining the room');
        }
    });
    ...
    socket.on('chatmsg', async ({ groupname, username, msg }) => {
        try {
            const ncreateuserOrAddToRoomewmsg = await createmsg(groupname, username, socket.id, msg);
            console.log(msg);
            socket.broadcast.emit(groupname, { user: username, message: msg, time: Date.now() });
        } catch (err) {
